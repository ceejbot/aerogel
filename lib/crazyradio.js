var
	...
	util              = require('util'),
	P                 = require('p-promise'),
	CRTPPacket        = require('./crazypacket'),
...
Crazyradio.prototype.scanChannels = function(start, stop, packet)
{
	...
	return self.usbSendVendor(SCAN_CHANNELS, start, stop, packet)
		.then(self.usbReceiveVendor(SCAN_CHANNELS, 0, 0, 64));
};
...
Crazyradio.prototype.statusRequest = function()
{
	...
	{
		console.log('statusRequest', data);
		if (err)
...
Crazyradio.prototype.usbReceiveVendor = function(request, value, index, length)
{
	...
	{
		console.log('usbReceiveVendor', err);
		if (err)
			return deferred.reject(err);
...
Crazyradio.prototype.onReadable = function()
{
	...
	var nextDeferred = this.queue.shift();
	if (nextDeferred)
		nextDeferred.resolve(buf);
};
...
Crazyradio.prototype.write = function(data)
{
	var self = this,
		deferred = P.defer();
...
/*
Crazyradio.prototype.sendPacket = function(data)
{
	console.log('entering sendPacket()', data);
	assert(data instanceof CRTPPacket, 'Crazyradio.sendPacket() requires a packet');
	var self = this,
		deferred = P.defer();

	this.outEndpoint.transfer(data, function(err)
	{
		console.log('out transfer: ', err);
		if (err)
			return deferred.reject(err);

		self.inEndpoint.transfer(64, function(err, data)
		{
			console.log('in transfer:', err, data);
			if (err)
				return deferred.reject(err);
			var ack = new RadioAck(data);
...
Crazyradio.CRTPPacket = CRTPPacket;
